...
# 1. Start SSE Stream in background (using query param auth for simplicity)
# We use -N (no buffer)
curl -N -s "${API_URL}/mcp/sse?apiKey=${API_KEY}" > "$SSE_OUT" &
CURL_PID=$!
...
# Append API key to POST URL as query param
if [[ "$FULL_POST_URL" == *"?"* ]]; then
    FULL_POST_URL="${FULL_POST_URL}&apiKey=${API_KEY}"
else
    FULL_POST_URL="${FULL_POST_URL}?apiKey=${API_KEY}"
fi
